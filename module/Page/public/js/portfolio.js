(function ($, window, document, undefined) {
    ...
    gridContainer.cubeportfolio({
        ...
        singlePageInlineCallback: function (url, element) {
            ...
            $.ajax({
                ...
                t.updateSinglePageInline(result.content);
            })
            .fail(function () {
    ...
    if($('.cbp-l-loadMore-button').length > 0)
    {
        $('.cbp-l-loadMore-button-link').on('click', function() {
            ...
            if(nextPage)
            {
                ...
                $.ajax({
                    ...
                    {
                        $('.cbp-wrapper').append(json.products);
                        refreshContent();
                        cache.text('Pokaż więcej');
                        cache.attr('id', json.nextPage);

                        if (json.countProducts == $('.cbp-item').length) {
                            cache.text('To już wszystko');
                            cache.removeAttr('id');
                        }
                    }
                });
